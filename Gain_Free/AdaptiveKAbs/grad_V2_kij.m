...
for h=1:N
    ...
    if(~isempty(fm{i,h}))
                DV2_dkij = DV2_dkij + abs(K_i)*B_i'*A_ih{h}'*A_ih{h}*B_i*sign(K_i)' - fm{i,h}'*A_ih{h}*B_i*sign(K_i)';
...
%           DV2_dkij = DV2_dkij + (A_ih{h}*B_i*1/2*K_i.^2' - fm{i,j})'*(A_ih{h}*B_i*K_i');
    ...
    if(~isempty(fm{j,h}))
        DV2_dkij = DV2_dkij + abs(K_j)*B_j'*A_jh{h}'*A_jh{h}*B_j*sign(K_j)' - fm{j,h}'*A_jh{h}*B_j*sign(K_j)';
