...
for h=1:N
    ...
    if(~isempty(fm{i,h}))
        DV2_dkij = DV2_dkij + 1/2*((K_i.^2)'*B_i'*A_ih{h}'*A_ih{h}*B_i*DK_i - fm{i,h}'*A_ih{h}*B_i*DK_i);
    end
    
    if(~isempty(fm{j,h}))
        DV2_dkij = DV2_dkij + 1/2*((K_j.^2)'*B_j'*A_jh{h}'*A_jh{h}*B_j*DK_j - fm{j,h}'*A_jh{h}*B_j*DK_j);
    end
